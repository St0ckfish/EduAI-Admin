...
const Chat = () => {
      ...
      if (isInitialMount.current) {
        ...
        setLocalChats(prevChats => {
          ...
          return updatedChats.map(newChat => {
            ...
            const existingChat = prevChats.find(chat => chat.chatId === newChat.chatId);
            if (existingChat) {
              return { ...newChat, numberOfNewMessages: existingChat.numberOfNewMessages };
            }
  ...
  const handleChatUpdate = useCallback((update: ChatData) => {
    ...
    setLocalChats((prevChats: ChatData[]) => {
        ...
        return [
          {
            ...
            numberOfNewMessages: isActiveChat ? 0 : (update.numberOfNewMessages || 1)
          }, 
          ...prevChats
        ];
      }
      ...
      console.log("Updating existing chat:", update.chatId, "Active:", isActiveChat);
      updatedChats[existingChatIndex] = {
        ...updatedChats[existingChatIndex],
        lastMessage: update.lastMessage || updatedChats[existingChatIndex].lastMessage,
        // If it's the active chat, don't increment the counter
        numberOfNewMessages: isActiveChat 
          ? 0 
          : (updatedChats[existingChatIndex].numberOfNewMessages || 0) + (update.numberOfNewMessages || 1),
        targetUser: {
      ...
      console.log("Updated chats:", updatedChats.map(c => c.chatId));
      return updatedChats;
    });
  }, [userId]);
                  ...
                  <div className="flex justify-between text-start text-[22px] font-semibold">
                    ...
                    <div className="flex items-center">
                      ...
                      <button 
                        ...
                        title={getTranslation("Create new chat", "إنشاء محادثة جديدة", "Créer une nouvelle discussion")}
                      >
                        <svg
                          className="h-8 w-8"
                          fill="none"
                    ...
                    <div>
                      ...
                      <div className="relative min-w-48 md:min-w-80">
                        ...
                        <input
                          ...
                          placeholder={getTranslation("Search", "بحث", "Recherche")}
                        />
                      </div>
                    </div>
                  </div>
                  <div className="mt-2 grid gap-2">
                      ...
                      .map((chat: ChatData) => (
                        ...
                        >
                          ...
                          <div className="grid w-full items-center justify-end gap-4 text-center text-end text-white">
                            <button onClick={(e) => handleOpenModal2(chat.chatId, e)}>
                              <svg
                    ...
                    {localChats.length === 0 && !isLoading && (
                      <div className="mt-10 text-center text-textSecondary">
                        <p>{getTranslation("No chats available", "لا توجد محادثات", "Pas de discussions disponibles")}</p>
                        <button 
                          onClick={handleOpenModal}
                          className="mt-2 rounded-lg bg-primary px-4 py-2 text-white hover:bg-hover"
                        >
                          {getTranslation("Start a new chat", "بدء محادثة جديدة", "Démarrer une nouvelle discussion")}
                        </button>
                      </div>
                    )}
                  </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              >
                ...
                <SearchableSelect
                  ...
                  placeholder={getTranslation("Select user", "اختر مستخدم", "Sélectionner un utilisateur")}
                />
              </label>
              <button
                disabled={isLoading}
              ...
              >
                {isLoading
                  ? getTranslation("Adding...", "يتم الإضافة...", "Ajout en cours...")
                  : getTranslation("Add Chat", "إضافة دردشة", "Ajouter un Chat")}
              </button>
            </form>
          )}
        </Modal>
