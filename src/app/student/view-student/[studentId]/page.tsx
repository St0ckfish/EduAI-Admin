...
import { useGetStudentByIdQuery, useGetStudentExamsQuery } from "@/features/User-Management/studentApi";
...
const ViewStudent: React.FC<ViewStudentProps> = ({ params }) => {
  ...
  const { data: exams, isLoading: isExams } = useGetStudentExamsQuery({ id: params.studentId, date: format(selectedDate || new Date(), 'yyyy-MM-dd') });
            ...
            <tbody>
              {
                exams?.data.map((exam: any) => (
                  <tr key={exam.id} className="border-b border-borderPrimary bg-bgPrimary hover:bg-bgSecondary">
                <th
                  scope="row"
