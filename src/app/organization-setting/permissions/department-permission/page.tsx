...
const DepartmentPermission = () => {
    ...
    const { data, error, isLoading } = useGetAllDepartmentPermissionPermissionsQuery(null);
    ...
    return ( 
        <>
            ...
            <div className="flex justify-left gap-5 text-[20px] max-[725px]:text-[15px] flex-wrap font-semibold mb-[80px] mt-[50px] ml-4">
                    <Link href="/organization-setting/permissions/department-permission" className="text-blue-500 underline">
                    Department 
                    </Link>
                ...
                <div className="flex justify-between max-[502px]:grid max-[502px]:justify-center text-center">
                    ...
                    <div className="flex justify-center">
                        <Link href="/organization-setting/permissions/add" className="px-4 py-2 whitespace-nowrap rounded-xl bg-[#3E5AF0] hover:bg-[#4a5cc5] hover:shadow-xl mb-5 mr-3 text-white text-[18px] ease-in font-semibold duration-300">+ Add Department Permissions</Link>
                    </div>
                </div>
                <div className="overflow-auto relative shadow-md sm:rounded-lg">
                    <table className="w-full overflow-x-auto text-sm text-left rtl:text-right text-gray-500 ">
                        ...
                        <tbody>
                        {data?.data.content.filter((departmentPermission: DepartmentPermission) => {
                            return search.toLocaleLowerCase() === '' ? departmentPermission : departmentPermission.name.toLocaleLowerCase().includes(search);
                        }).map((departmentPermission: DepartmentPermission) => (
                            <tr key={departmentPermission.id} className="bg-white border-b  hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <Link href={`/organization-setting/permissions/department-permission/${departmentPermission.id}`} className="font-medium text-blue-600 hover:underline">edit</Link>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
