...
import { useDeletePositionsMutation, useGetAllPositionsQuery } from "@/features/Organization-Setteings/positionApi";
    ...
    return ( 
        <>
            <div className={`flex items-center gap-1 ${booleanValue ? "lg:ml-[100px]" : "lg:ml-[270px]"} mt-12 ml-7 text-[18px] max-[550px]:text-[15px]  flex-wrap`}>
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style={{ fill: 'rgba(82, 100, 132, 1)', transform: '', msFilter: '' }}><path d="M10.707 17.707 16.414 12l-5.707-5.707-1.414 1.414L13.586 12l-4.293 4.293z"></path></svg>
                <Link className="text-[#526484] hover:text-blue-400 hover:underline  font-semibold" href="/position">Position</Link>
            </div>
            <div className={`${booleanValue ? "lg:ml-[100px]" : "lg:ml-[270px]"} mr-[5px] relative mt-10 overflow-x-auto bg-transparent sm:rounded-lg h-screen`}>
                <div className="flex justify-between max-[502px]:grid max-[502px]:justify-center text-center">
                    ...
                    <div className="flex justify-center">
                        <Link href="/organization-setting/position/add-position" className="px-4 py-2 whitespace-nowrap rounded-xl bg-[#3E5AF0] hover:bg-[#4a5cc5] hover:shadow-xl mb-5 mr-3 text-white text-[18px] w-[180px] ease-in font-semibold duration-300">+ Add Position</Link>
                    </div>
                </div>
                <div className="overflow-auto relative shadow-md sm:rounded-lg">
                    <table className="w-full overflow-x-auto text-sm text-left rtl:text-right text-gray-500 ">
                        ...
                        {data?.data.content.filter((Position: Position) => {
                            return search.toLocaleLowerCase() === '' ? Position : Position.name.toLocaleLowerCase().includes(search);
                        }).map((Position: Position) => (
                            <tr key={Position.id} className="bg-white border-b  hover:bg-gray-50">
                                    ...
                                    <div className="w-[50px]">
                                        {
                                            ...
                                            :
                                            <img src={Position.picture} className="w-[40px] h-[40px] mr-2 rounded-full" alt="#" />
                                        }
                                    </div>
                                    <p> {Position.name} </p>
                                </th>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <Link href={`/organization-setting/department/${Position.id}`} className="font-medium text-blue-600 hover:underline">View</Link>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <button disabled={isDeleting} onClick={() => handleDelete(Position.id)} className="px-2 py-1 rounded-lg text-white bg-red-500 font-semibold shadow-lg ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300">Delete</button>
                                </td>
                            </tr>
                            ))}
                        </tbody>
