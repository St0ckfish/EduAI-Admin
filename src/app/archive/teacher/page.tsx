...
import { useGetAllTeachersQuery, useDeleteTeachersMutation } from "@/features/User-Management/teacherApi";
    ...
    return ( 
        <>
            <div className={`flex items-center gap-1 ${booleanValue ? "lg:ml-[100px]" : "lg:ml-[270px]"} mt-12 ml-7 text-[18px] max-[550px]:text-[15px]  flex-wrap`}>
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style={{ fill: 'rgba(82, 100, 132, 1)', transform: '', msFilter: '' }}><path d="M10.707 17.707 16.414 12l-5.707-5.707-1.414 1.414L13.586 12l-4.293 4.293z"></path></svg>
                <Link className="text-[#526484] hover:text-blue-400 hover:underline  font-semibold" href="/archive//teacher">Teacher</Link>
            </div>
                    ...
                    <table className="w-full overflow-x-auto text-sm text-left rtl:text-right text-gray-500 ">
                       ...
                       {data?.data.content.filter((teacher: Teacher) => {
                            ...
                            <tr key={teacher.id} className="bg-white border-b  hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <button onClick={()=> handleDelete(teacher.id)} className="px-2 py-1 rounded-lg text-white bg-red-500 font-semibold shadow-lg ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300">Unlock</button>
                                </td>
                            </tr>
                            ))}
                        </tbody>
