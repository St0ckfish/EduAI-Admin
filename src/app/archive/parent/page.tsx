    ...
    return (
        <>
            ...
            <Link className="text-[#526484] hover:text-blue-400 hover:underline  font-semibold" href="/">Administration</Link>
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" style={{ fill: 'rgba(82, 100, 132, 1)', transform: '', msFilter: '' }}><path d="M10.707 17.707 16.414 12l-5.707-5.707-1.414 1.414L13.586 12l-4.293 4.293z"></path></svg>
                <Link className="text-[#526484] hover:text-blue-400 hover:underline  font-semibold" href="/archive/parent">Parent</Link>
            </div>
                ...
                <div className="overflow-auto relative shadow-md sm:rounded-lg">
                    <table className="w-full overflow-x-auto text-sm text-left rtl:text-right text-gray-500 ">
                        ...
                        {data?.data.content.filter((parent: Parent) => {
                            return search.toLocaleLowerCase() === '' ? parent : parent.name.toLocaleLowerCase().includes(search);
                        }).map((parent: Parent) => (
                            <tr key={parent.id} className="bg-white border-b  hover:bg-gray-50">
                                ...
                                <div className="w-[50px]">
                                        {
                                            ...
                                            :
                                            <img src={parent.picture} className="w-[40px] h-[40px] mr-2 rounded-full" alt="#" />
                                        }
                                    </div>
                                    <p> {parent.name} </p>
                                </th>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <button onClick={()=> handleDelete(parent.id)} className="px-2 py-1 rounded-lg text-white bg-red-500 font-semibold shadow-lg ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300">Unlock</button>
                                </td>
                            </tr>
                            ))}
                        </tbody>
