import { createApi, fetchBaseQuery } from "@reduxjs/toolkit/query/react";
import { baseUrl } from "@/components/BaseURL";

const getCookie = (name: string) => {
  const value = `; ${document.cookie}`;
  const parts = value.split(`; ${name}=`);
  if (parts.length === 2) return parts.pop()?.split(";").shift();
  return null;
};

const getTokenFromCookie = () => {
  return getCookie("token");
};

export const complainApi = createApi({
  reducerPath: "complainApi",
  baseQuery: fetchBaseQuery({
    baseUrl: baseUrl,
    prepareHeaders: headers => {
      const token = getTokenFromCookie();

      if (token) {
        headers.set("Authorization", `Bearer ${token}`);
      }

      return headers;
    },
  }),
  endpoints: builder => ({
    getAllComplains: builder.query({
      query: ({ approved, type }) =>
        `/api/v1/management/complain/all?size=1000000&page=0&deleted=false&type=${type}&approved=${approved}`,
    }),
    //
    deleteComplains: builder.mutation({
      query: id => ({
        url: `/api/v1/management/complain/${id}`,
        method: "DELETE",
      }),
    }),
    //
    createComplains: builder.mutation({
      query: formData => ({
        url: `/api/v1/management/complain/`,
        method: "POST",
        body: formData,
      }),
    }),
    //
    getComplainById: builder.query({
      query: id => `/api/v1/management/complain/${id}`,
    }),
    //
    updateComplains: builder.mutation({
      query: ({ formData, id }) => ({
        url: `/api/v1/management/complain/${id}`,
        method: "PATCH",
        body: formData,
      }),
    }),
  }),
});

export const {
  useGetAllComplainsQuery,
  useDeleteComplainsMutation,
  useCreateComplainsMutation,
  useGetComplainByIdQuery,
  useUpdateComplainsMutation,
} = complainApi;
